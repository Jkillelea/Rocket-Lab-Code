function[SM_results1,SM_results3] = RUN_CEA(Pc_en, AR_sup)
    ...
    %% Run CEA - Still need this line since it creates the input file
    SM_inputs.runCEA(); % execute CEA for the above conditions

    % Make a call to the remote server
    remote_cea('./CEA Code/Detn.inp', './CEA Code/Detn.out');
    

    % 1 for chamber conditions (2 for throat, 3 for nozzle exit: requires arg 
    % for area ratio or exit pressure), 'en' for english units
    SM_results1 = SM_inputs.getCEAresults(1, 'si');
    SM_results3 = SM_inputs.getCEAresults(3, 'si');
end
